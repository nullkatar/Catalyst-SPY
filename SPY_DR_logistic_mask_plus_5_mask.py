...
Mask    = "Mask_Ada"
#Mask    = "Mask_Cheat"
#ProblemFile = "Lasso_Simpl

FileData = "madelon_data"
FileLabels = "madelon_data"
Lambda1 = 0.001
ITE_MAX = 3000
Debug = True
...
if rank == 0:
    MasterData = "./data/" + FileData + ".dat"
    MasterLabels = "./data/" + FileLabels + ".dat"
else:
    SlaveData = "./data/slaves/" + FileData + "_worker_" + str(rank) + ".dat"
    SlaveLabels = "./data/slaves/" + FileLabels + "_worker_" + str(rank) + ".dat"
...
if rank == 0:
    Data1 = np.genfromtxt(MasterData)
    Data2 = np.genfromtxt(MasterLabels)

    for i in range(len(Data1)):
        Data1[i] = [Data1[i][j]/np.linalg.norm(Data1[i]) for j in range(len(Data1[i]))]
...
if rank != 0:
    ...
    Data1 = np.genfromtxt(SlaveData)
    Data2 = np.genfromtxt(SlaveLabels)
...
if rank == 0:
    k = 0
    StartTime = time.time()
    #X =  open("./output/X_CATA_Ada.dat", 'r')
    #Y =  open("./output/Y_CATA_Ada.dat", 'r')
    k = 0
    barx = np.ones(n)#np.array([float(iter) for iter in (list(X)[-1]).split()])
    y_s = np.ones(n)#np.array([float(iter) for iter in (list(Y)[-1]).split()])
    #X.close()
    #Y.close()
...
#            S_bar = [i for i in list(range(n)) if i not in buf_ind]
        ...
        x_i[S] = barx[S] - gam * f_grad(csr_matrix(barx).T, A, b, Lambda2).T.toarray()[0][S]
        
        if Mask == "Mask_Part" or Mask == "Mask_Ada":
            x_i[buf_ind] = p * x_i[buf_ind] + (1 - p) * x_i_old[buf_ind]
...
if rank == 0:
    out_file = open("./output/X_SPY_Ada_madelon.dat", 'a')
    out_file.writelines("%s " % place1 for place1 in x)                                                   #Writing X
    out_file.writelines('\n')
    out_file.close()

    out_file = open("./output/times_SPY_Ada_madelon.dat", 'a')
    out_file.writelines("%s " % place2 for place2 in time_tab)                                            #Writing Times
    out_file.writelines('\n')
    out_file.close()
    
    out_file = open("./output/func_SPY_Ada_madelon.dat", 'a')
    for i in range(len(val_tab)):
        out_file.writelines("%s " %  function(val_tab[i]))                                                #Writing Obj Function
        out_file.writelines('\n')
    out_file.close()

    out_file = open("./output/exchanges_SPY_Ada_madelon.dat", 'a')
    out_file.writelines("%s " % place2 for place2 in ex_tab)                                              #Writing Exchanges
    out_file.writelines('\n') 
    out_file.close()

    out_file = open("./output/sparsity_SPY_Ada_madelon.dat", 'a')
    out_file.writelines("%s " % place2 for place2 in sp_tab)                                              #Writing Exchanges
    out_file.writelines('\n')
    out_file.close()
