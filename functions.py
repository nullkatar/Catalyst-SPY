...
def f(x, A, b, lam2):#, y, kappa):
    l = np.log(1 + np.exp(-A.dot(x).multiply(b).A))
    m = b.shape[0]
    return np.sum(l) / m + lam2/2 * np.linalg.norm(x.toarray()) ** 2 #+ kappa * l1(x - y, 1) ** 2

def f_grad(x, A, b, lam2, y, kappa):
    assert ((b.shape[0] == A.shape[0]) & (x.shape[0] == A.shape[1]))
    assert lam2 >= 0
    denom = csr_matrix(1/(1 + np.exp(A.dot(x).multiply(b).A)))
    g = -(A.multiply(b).multiply(denom).sum(axis=0).T)
    m = b.shape[0]
    return csr_matrix(g) / m + lam2 * x + kappa * eye(y.shape[0]).T.dot(eye(y.shape[0]).dot(x) - y) #kappa * csr_matrix(x - y)
